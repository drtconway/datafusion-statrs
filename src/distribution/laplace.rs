...
/// ScalarUDF for the Laplace Distribution PDF
pub fn pdf() -> ScalarUDF {
    ScalarUDF::from(Pdf::new("laplace_pdf"))
}

type LnPdf = Continuous3F<LnPdfEvaluator3F<Laplace>>;

/// ScalarUDF for the Laplace Distribution log PDF
pub fn ln_pdf() -> ScalarUDF {
    ScalarUDF::from(LnPdf::new("laplace_ln_pdf"))
}

type Cdf = Continuous3F<CdfEvaluator3F<Laplace>>;

/// ScalarUDF for the Laplace Distribution CDF
...
/// ScalarUDF for the Laplace Distribution SF
pub fn sf() -> ScalarUDF {
    ScalarUDF::from(Sf::new("laplace_sf"))
}

/// Register the functions for the Laplace Distribution
...
mod tests {
    ...
    use statrs::distribution::LaplaceError;
    ...
    async fn laplace_pdf_success() {
        ...
        assert_eq_float!(res_col.value(0), 0.0006709252558050237);
        assert_eq_float!(res_col.value(1), 0.03663127777746836);
        assert!(res_col.value(2).is_nan());
        assert!(res_col.value(3).is_nan());
    }

    #[tokio::test]
    async fn laplace_pdf_failure_1() {
        let pdf = pdf();
        ...
        match res {
            Err(DataFusionError::External(e)) => {
                let be = e.downcast::<LaplaceError>().unwrap();
                assert_eq!(*be.as_ref(), LaplaceError::ScaleInvalid);
            }
            _ => {
    ...
    #[tokio::test]
    async fn laplace_ln_pdf_success() {
        let mut ctx = SessionContext::new();
        register(&mut ctx).unwrap();
        let res = ctx
            .sql("SELECT laplace_ln_pdf(0.2, 8.0, 11.0)")
            .await
            .unwrap()
            .collect()
            .await
        ...
        let res_col = as_float64_array(res[0].column(0)).unwrap();
        assert_eq_float!(res_col.value(0), -3.800133362449225);
    }

    #[tokio::test]
    async fn laplace_cdf_success() {
        let pdf = cdf();
        ...
        assert_eq!(res[0].num_rows(), 4);
        let res_col = as_float64_array(res[0].column(0)).unwrap();
        assert_eq_float!(res_col.value(0), 0.00016773131395125593);
        assert_eq_float!(res_col.value(1),  0.00915781944436709);
        assert!(res_col.value(2).is_nan());
        assert!(res_col.value(3).is_nan());
    }

    #[tokio::test]
    async fn laplace_sf_success() {
        let pdf = sf();
        ...
        assert_eq!(res[0].num_rows(), 4);
        let res_col = as_float64_array(res[0].column(0)).unwrap();
        assert_eq_float!(res_col.value(0), 0.9998322686860488);
        assert_eq_float!(res_col.value(1), 0.9908421805556329);
        assert!(res_col.value(2).is_nan());
        assert!(res_col.value(3).is_nan());
    }
}
